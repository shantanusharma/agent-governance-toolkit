...
AGENTS = [
    {
        "name": "payment-agent",
        "did": "did:web:payments.mesh.io",
        "protocol": "A2A",
        "trust_score": 920,
        "dimensions": [95, 92, 88, 96, 90],
    },
    {
        "name": "customer-service",
        "did": "did:web:cs.mesh.io",
        "protocol": "A2A",
        "trust_score": 870,
        "dimensions": [88, 90, 85, 82, 92],
    },
    {
        "name": "data-analyst",
        "did": "did:web:analytics.mesh.io",
        "protocol": "MCP",
        "trust_score": 810,
        "dimensions": [90, 78, 92, 80, 85],
    },
    {
        "name": "fraud-detector",
        "did": "did:web:fraud.mesh.io",
        "protocol": "IATP",
        "trust_score": 940,
        "dimensions": [96, 95, 90, 98, 94],
    },
    {
        "name": "inventory-manager",
        "did": "did:web:inventory.mesh.io",
        "protocol": "MCP",
        "trust_score": 720,
        "dimensions": [75, 70, 82, 68, 78],
    },
    {
        "name": "email-dispatcher",
        "did": "did:web:email.mesh.io",
        "protocol": "A2A",
        "trust_score": 650,
        "dimensions": [60, 72, 80, 55, 68],
    },
    {
        "name": "auth-gateway",
        "did": "did:web:auth.mesh.io",
        "protocol": "IATP",
        "trust_score": 950,
        "dimensions": [98, 96, 94, 99, 95],
    },
    {
        "name": "report-generator",
        "did": "did:web:reports.mesh.io",
        "protocol": "MCP",
        "trust_score": 580,
        "dimensions": [55, 62, 70, 50, 60],
    },
    {
        "name": "scheduler",
        "did": "did:web:scheduler.mesh.io",
        "protocol": "A2A",
        "trust_score": 780,
        "dimensions": [80, 76, 85, 74, 82],
    },
    {
        "name": "compliance-bot",
...
def _build_protocol_traffic(hours: int = 24) -> pd.DataFrame:
    ...
    for t in timestamps:
        for proto in ["A2A", "MCP", "IATP"]:
            ...
            rows.append(
                {"timestamp": t, "protocol": proto, "messages": int(base + rng.integers(-20, 20))}
            )
    return pd.DataFrame(rows)
...
def _build_audit_log(n: int = 20) -> pd.DataFrame:
    ...
    for i in range(n):
        rows.append(
            {
                "timestamp": (NOW - dt.timedelta(minutes=int(rng.integers(1, 1440)))).strftime(
                    "%Y-%m-%d %H:%M"
                ),
...
# Filtered agent list
filtered = [
    a for a in AGENTS if a["name"] in selected_agents and a["protocol"] in selected_protocols
]
...
tab_net, tab_scores, tab_creds, tab_proto, tab_compliance = st.tabs(
    [
        "\U0001f310 Trust Network",
        "\U0001f4ca Trust Scores",
        "\U0001f512 Credential Lifecycle",
        "\U0001f4e1 Protocol Traffic",
        "\u2705 Compliance",
    ]
)
...
with tab_scores:
    ...
    fig_bar = px.bar(
        ...
        range_color=[0, 1000],
        labels={"name": "Agent", "trust_score": "Trust Score"},
        title="Trust Score Leaderboard",
    )
    fig_bar.update_layout(
        template=PLOTLY_TEMPLATE, height=400, yaxis=dict(categoryorder="total ascending")
    )
    ...
    fig_decay.update_layout(
        ...
        xaxis_title="Hours Without Activity",
...
with tab_creds:
    ...
    for i, row in timeline_df.iterrows():
        color = {"active": "#388e3c", "expired": "#d32f2f", "expiring-soon": "#fbc02d"}[
            row["status"]
        ]
    ...
    with col_pairs:
        ...
        if pair_data:
            ...
            fig_pairs.update_layout(
                template=PLOTLY_TEMPLATE, height=380, yaxis=dict(categoryorder="total ascending")
            )
            ...
            chain_rows.append(
                {
                    ...
                    "last_check": (NOW - dt.timedelta(minutes=int(rng.integers(5, 120)))).strftime(
                        "%H:%M"
                    ),
                }
            )
