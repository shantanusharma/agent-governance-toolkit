...
class GovernedMCPServer:
    ...
    def _register_tools(self):
        ...
        async def filesystem_read(params: Dict[str, Any]) -> str:
            ...
            # Policy check
            policy_result = self._check_policy(action="filesystem_read", params={"path": path})
        ...
        async def database_query(params: Dict[str, Any]) -> list:
            ...
            # Policy check
            policy_result = self._check_policy(action="database_query", params={"query": query})
        ...
        async def api_call(params: Dict[str, Any]) -> Dict[str, Any]:
            ...
            policy_result = self._check_policy(
                action="api_call", params={"endpoint": endpoint, "method": method}
            )
            ...
            # Simulate API call
            response = {"status": 200, "data": {"message": f"Response from {endpoint}"}}
    ...
    def _check_policy(self, action: str, params: Dict[str, Any]) -> Dict[str, bool]:
        ...
        if action == "filesystem_read":
            ...
            if any(path.startswith(p) for p in sensitive_paths):
                return {"allowed": False, "reason": f"Access to {path} is blocked by policy"}
        ...
        if action == "database_query":
            ...
            if any(keyword in query_upper for keyword in dangerous_keywords):
                return {"allowed": False, "reason": f"Dangerous SQL keywords detected in query"}
    ...
    async def demo_tool_invocations(self):
        ...
        result = await self.mcp_server.handle_request(
            "filesystem_read", {"path": "/data/users.json"}
        )
        print(f"   Result: {result}")

        # Test 2: Blocked filesystem read
        print("\n2️⃣  Test: Reading blocked file")
        result = await self.mcp_server.handle_request("filesystem_read", {"path": "/etc/passwd"})
        print(f"   Result: {result}")
        ...
        print("\n5️⃣  Test: API call")
        result = await self.mcp_server.handle_request(
            "api_call", {"endpoint": "https://api.weather.com/current", "method": "GET"}
        )
        print(f"   Result: {result}")
