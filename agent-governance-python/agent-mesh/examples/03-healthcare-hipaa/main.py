...
class HealthcareAgent:
    ...
    async def access_patient_data(self, patient_id: str, purpose: str) -> Dict[str, Any]:
        ...
        policy_result = self.policy_engine.check(
            action="access_phi", resource=f"patient:{patient_id}", purpose=purpose
        )
...
async def demo_healthcare_agent():
    ...
    try:
        data = await agent.access_patient_data(patient_id="P-12345", purpose="risk_assessment")
...
async def main():
    ...
    parser.add_argument(
        "--compliance-report", action="store_true", help="Generate compliance report only"
    )
